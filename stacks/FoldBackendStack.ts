...
import { Architecture, Code, Handler, LayerVersion, Runtime, StartingPosition } from 'aws-cdk-lib/aws-lambda';
...
const lambdaSecretsLayerArn =
	'arn:aws:lambda:us-east-1:177933569100:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:4' as const;
...
export function FoldBackendStack({ app, stack }: StackContext) {
	...
	const secretsCacheLayer = LayerVersion.fromLayerVersionArn(
		stack,
		'ProjectsSecretsLambdaLayer',
		lambdaSecretsLayerArn,
	);
	...
	if (IS_LOCAL) {
		console.warn(
			`WARN: Adding network ingress rule to allow DB to be accessed from your public IP address (${publicIp})`,
		);
		sg.addIngressRule(
	...
	if (IS_LOCAL) {
		console.warn(
			`WARN: Adding database connection rule to allow DB to be accessed from your public IP address (${publicIp})`,
		);
	...
	streamHandler.addEventSource(
		new KinesisEventSource(streamConstruct, {
	...
	const dmsVpcIamRole = new Role(stack, 'ProjectsDMSVPCManageIamRole', {
		...
		managedPolicies: [
			...
			ManagedPolicy.fromManagedPolicyArn(
				stack,
				'ProjectsDMSVPCManagedPolicyLookup',
	...
	openSearchMasterSecret.grantRead(
		Role.fromRoleArn(stack, 'ProjectsApiHandlerExecRoleLookup', apiFunction.role?.roleArn as string),
	);
	...
	openSearchMasterSecret.grantRead(
		Role.fromRoleArn(stack, 'ProjectsStreamHandlerExecRoleLookup', streamHandler.role?.roleArn as string),
	);
	...
	dbMasterSecret.grantRead(
		Role.fromRoleArn(stack, 'ProjectsDbInitHandlerExecRoleLookup', dbInit.role?.roleArn as string),
	);
