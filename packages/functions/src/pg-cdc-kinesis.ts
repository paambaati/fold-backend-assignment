...
export const main: KinesisStreamHandler = async (event) => {
	...
	const { OPENSEARCH_DOMAIN_ENDPOINT: osEndpoint, OPENSEARCH_MASTER_CREDENTIALS_SECRET_ID: osSecretId } = envSchema
		.parse(process.env);
	...
	if (osOps.length) {
		console.info(
			`Going to execute ${osOps.length} operation(s) (out of ${cdcRecords.length} records total) on OpenSearch...`,
		);
		...
		for (const [index, osResult] of osResults.entries()) {
			...
			console.info(
				`OpenSearch response for ${opMeta.operationType} (HTTP ${opMeta.method}) operation on ${opMeta.index}/${opMeta.recordId}`,
				osResult.statusCode,
				osResult.body,
			);
		}
