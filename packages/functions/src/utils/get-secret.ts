 ...
 */
const getSecret = async (secretId: string, version?: MergeExclusive<{versionId: string}, {versionStage: string}>): Promise<string | undefined> => {
    if (!secretId) {
    ...
    if (!sessionToken || !port) {
        console.warn(`Secrets Manager cache layer not configured${!port ? ' (correctly)' : ''}, falling back to getting secret via API from Secrets Manager.`);
        console.warn('Tip: Consider adding a cache layer to improve your Lambda response times! See https://docs.aws.amazon.com/secretsmanager/latest/userguide/retrieving-secrets_lambda.html for more information.');
 ...
 */
export const getCredentials = async (secretId: string, version?: MergeExclusive<{versionId: string}, {versionStage: string}>): Promise<Credentials> => {
